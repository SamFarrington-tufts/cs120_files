  ...
  if (path == "/"){
    ...
    MongoClient.connect(mongoUrl, async function(err, db) {
        if(err){
            ...
            // creates a query variable and assigns the zip or place name to it. determines
            // if the user's input is the place name or zip by assessing the first char of the
            // input
            let query;
            ...
            await collection.find(query).toArray(function(err, items){
                if(err){
                    ...
                    if(items.length > 0){
                        // prints the name of the place to the page and console
                        let result = items[0];
                        res.write(result.place + "<br />");
                        console.log(result.place);
                        
                        // prints every zip associated to a place on a new line on the webpage
                        // and the console
